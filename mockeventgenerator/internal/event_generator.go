...
func (eg *EventGenerator) RunEventGeneration(sendFunc func([]common.Event)) {
...
func (eg *EventGenerator) GenerateRandomEvents() []common.Event {
	...
	if numCreatedUsers == 0 {
		return []common.Event{eg.eventFactory.CreateUserEvent()}
	}
...
func (eg *EventGenerator) createPairOfBetEvents(userID uint) []common.Event {
	var events []common.Event
	betEvent := eg.eventFactory.CreateBetEvent(userID, common.EventTypeBet)
	events = append(events, betEvent)

	winOrLossTypes := []common.EventType{common.EventTypeWin, common.EventTypeLoss}
